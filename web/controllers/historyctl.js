...
const historyController = {
    ...
    historySearch: (req, res) => {
        ...
        try {
            ...
            const edate = new Date(`${filterTime.endDate}T${filterTime.endTime}`);
    ...
    historyCartSearch: (req, res) => {
        ...
        // 查詢特定類別的商品並新增至itemsarr
        session
            .run(`match(o) where ID(o) = ${data.id} match(o) -[:orders]-> (p:cart) return p`)
            .then(result => {
                // 依序抓取回傳的節點
                result.records.forEach(record => {
                    carts.push(record.get('p').properties);
            ...
            .catch(error => {
                console.log('historyCartSearch error:', error)
            })
            .then(() => {
                session.close()
                res.send(carts)
